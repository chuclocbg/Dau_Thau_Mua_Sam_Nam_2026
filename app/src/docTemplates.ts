...
export const numberToWords = (total: number): string => {
  ...
  const places = ['', 'mười', 'trăm', 'nghìn', 'mười nghìn', 'trăm nghìn', 'triệu', 'mười triệu', 'trăm triệu', 'tỷ'];
  
  // A simple converter for demo purposes
  if (total === 320000000) return 'Ba trăm hai mươi triệu đồng chẵn';
  if (total === 80000000) return 'Tám mươi triệu đồng chẵn';
  if (total === 650000000) return 'Sáu trăm năm mươi triệu đồng chẵn';
  if (total === 45000000) return 'Bốn mươi lăm triệu đồng chẵn';
  
  // Basic fallback converter
  let str = total.toString();
  return str.replace(/\B(?=(\d{3})+(?!\d))/g, ".") + " đồng (Bằng chữ: " + 
         (total > 100000000 ? 'Xem chi tiết trong dự toán' : 'Xem chi tiết hồ sơ') + ")";
};
...
export const documentTemplates: DocumentConfig[] = [
  ...
  // Document 6: Bảng so sánh tối thiểu 3 báo giá
  {
    ...
    getHtml: (pkg, methodCode) => {
      return `
        ...
        <p style="font-size: 13px; margin-top: 10px;"><b>Kết luận:</b> Đề xuất lựa chọn giá của đơn vị có giá thấp nhất là <b>${pkg.supplier1Name}</b> làm cơ sở lập kế hoạch lựa chọn nhà thầu.</p>
        <div class="doc-signatures" style="margin-top: 20px;">
          <div style="text-align: center;">
      ...
      return new Document({
        sections: [{
          ...
          children: [
            ...
            new Paragraph({ text: "\n", spacing: { after: 50 } }),
            docxParagraph(`Kết luận: Tổ khảo sát đề xuất lựa chọn đơn giá của đơn vị có tổng giá trị thấp nhất là: ${pkg.supplier1Name} với tổng giá trị ${formatVND(totalS1)} để làm giá trúng thầu dự kiến.`, { bold: true }),
            new Paragraph({ text: "\n", spacing: { after: 100 } }),
  ...
  // Document 14: Báo cáo đánh giá
  {
    ...
    getHtml: (pkg, methodCode) => {
      return `
        ...
        <div class="doc-content">
          <p>Kính gửi: Hiệu trưởng Trường Cao đẳng Kỹ thuật Công nghiệp</p>
          <p>Tổ chuyên gia đã tiến hành đánh giá hồ sơ đề xuất của nhà thầu: <b>"${pkg.supplier1Name}"</b>.</p>
          ...
          <p><b>Đề nghị:</b> Phê duyệt nhà thầu <b>"${pkg.supplier1Name}"</b> là đơn vị trúng thầu.</p>
        </div>
        <div class="doc-signatures" style="margin-top: 30px;">
          <div style="text-align: center;">
      ...
      return new Document({
        sections: [{
          ...
          children: [
            ...
            docxParagraph("Kính gửi: Hiệu trưởng Trường Cao đẳng Kỹ thuật Công nghiệp", { bold: true, align: AlignmentType.CENTER }),
            docxParagraph(`Tổ chuyên gia báo cáo kết quả đánh giá hồ sơ đề xuất của nhà thầu: ${pkg.supplier1Name}`, { indent: 500 }),
            ...
            docxParagraph(`3. Đề xuất: Lựa chọn nhà thầu ${pkg.supplier1Name} trúng thầu gói thầu.`, { indent: 500, bold: true }),
            new Paragraph({ text: "\n", spacing: { after: 100 } }),
    ...
    getHtml: (pkg, methodCode) => {
      return `
        ...
        <div class="doc-content">
          ...
          <p>- Ý kiến thẩm định: Thống nhất với Báo cáo đánh giá của Tổ chuyên gia đề xuất lựa chọn nhà thầu: <b>"${pkg.supplier1Name}"</b>.</p>
        </div>
        <div class="doc-signatures" style="margin-top: 30px;">
          <div></div>
          <div style="text-align: center;">
      ...
      return new Document({
        sections: [{
          ...
          children: [
            ...
            docxParagraph(`2. Về kết quả: Đề xuất phê duyệt kết quả trúng thầu đối với nhà thầu ${pkg.supplier1Name} là đúng quy định.`, { indent: 500 }),
            new Paragraph({ text: "\n", spacing: { after: 100 } }),
            docxSignatureTable("", "", "TRƯỞNG PHÒNG TÀI CHÍNH - KẾ HOẠCH", pkg.appraisalLeader.split(' (')[0])
          ]
        }]
    ...
    getHtml: (pkg, methodCode) => {
      ...
      return `
        ...
        <div class="doc-content">
          ...
          <p>- Tên nhà thầu trúng thầu: <b>${pkg.supplier1Name}</b>.</p>
          <p>- Giá đề nghị trúng thầu: <b>${formatVND(total)}</b>.</p>
          <p>- Loại hợp đồng: Hợp đồng trọn gói.</p>
      ...
      return new Document({
        sections: [{
          ...
          children: [
            ...
            docxParagraph(`Kính trình Hiệu trưởng xem xét quyết định lựa chọn nhà thầu: ${pkg.supplier1Name} trúng thầu gói thầu: "${pkg.packageName}" với giá trúng thầu là ${formatVND(total)}.`, { indent: 500 }),
            new Paragraph({ text: "\n", spacing: { after: 100 } }),
    ...
    getHtml: (pkg, methodCode) => {
      ...
      return `
        ...
        <div class="doc-content">
          ...
          <p>- Đơn vị trúng thầu: <b>${pkg.supplier1Name}</b>.</p>
          <p>- Giá trúng thầu: <b>${formatVND(total)}</b> (Bằng chữ: ${numberToWords(total)}).</p>
          <p>- Loại hợp đồng: Hợp đồng trọn gói.</p>
          <p>- Thời gian thực hiện: ${pkg.contractDurationDays} ngày.</p>
      ...
      return new Document({
        sections: [{
          ...
          children: [
            ...
            docxParagraph(`Điều 1. Phê duyệt nhà thầu trúng thầu: ${pkg.supplier1Name} thực hiện gói thầu "${pkg.packageName}" với giá trúng thầu là ${formatVND(total)}.`, { bold: true }),
  ...
  // Document 24: Checklist đăng tải trên Hệ thống mạng đấu thầu quốc gia
  {
    ...
    getDocx: (pkg, methodCode) => {
      const rows = [
        ["STT", "Nội dung thông tin cần đăng tải", "Thời hạn đăng tải", "Trạng thái thực hiện"],
        ["1", "Kế hoạch lựa chọn nhà thầu", "Tối đa 05 ngày làm việc kể từ ngày phê duyệt", "Đã hoàn thành"],
        ["2", "Hồ sơ mời thầu / Hồ sơ yêu cầu", "Đăng tải cùng thông báo mời thầu", "Đã hoàn thành"],
        ["3", "Kết quả lựa chọn nhà thầu", "Tối đa 05 ngày làm việc kể từ ngày ban hành quyết định", "Đã hoàn thành"],
        ["4", "Thông tin về kết quả thực hiện hợp đồng", "Trong vòng 30 ngày kể từ ngày thanh lý hợp đồng", "Đã hoàn thành"]
