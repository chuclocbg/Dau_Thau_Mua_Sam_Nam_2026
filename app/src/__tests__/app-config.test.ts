...
describe('loadAppConfigFromEnv — defaults', () => {
  it('returns sane defaults when no env vars are set', () => {
    ...
    expect(result).toEqual({
      ok: true,
      value: { port: 3000, host: '0.0.0.0', nodeEnv: 'development', logLevel: 'info', shutdownTimeoutMs: 10_000 },
    })
  })
})

describe('loadAppConfigFromEnv — overrides', () => {
  it('reads PORT/HOST/NODE_ENV/LOG_LEVEL/SHUTDOWN_TIMEOUT_MS from the given env map', () => {
    const result = loadAppConfigFromEnv({
      PORT: '8080', HOST: '127.0.0.1', NODE_ENV: 'production', LOG_LEVEL: 'debug', SHUTDOWN_TIMEOUT_MS: '5000',
    })
    expect(result).toEqual({
      ok: true,
      value: { port: 8080, host: '127.0.0.1', nodeEnv: 'production', logLevel: 'debug', shutdownTimeoutMs: 5000 },
    })
  })
})
