...
import type { AuditEntry, AuditTrailPanelProps } from '../components/AuditTrailPanel';
...
describe('AT1 · Component structure', () => {
  it('AT1-01: renders the audit-trail-panel wrapper element', () => {
    const html = renderToString(React.createElement<AuditTrailPanelProps>(AuditTrailPanel, {}));
  ...
  it('AT1-03: renders a custom title', () => {
    const html = renderToString(
      React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { title: 'Procurement Audit Log' }),
    );
  ...
  it('AT1-04: custom title overrides the default', () => {
    const html = renderToString(
      React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { title: 'My Log' }),
    );
  ...
  it('AT1-05: renders an ordered list element (ol.audit-entries) when entries exist', () => {
    const html = renderToString(
      React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { entries: [FULL_ENTRY] }),
    );
...
describe('AT2 · traceId rendering', () => {
  ...
  it('AT2-03: omits trace-id span when traceId is absent', () => {
    ...
    const html = renderToString(
      React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { entries: [entry] }),
    );
  ...
  it('AT2-04: renders multiple entries each with their own traceId', () => {
    ...
    const html = renderToString(
      React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { entries }),
    );
...
describe('AT10 · Empty and null state', () => {
  ...
  it('AT10-03: null entries prop renders empty-state', () => {
    const html = renderToString(
      React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { entries: null }),
    );
    expect(html).toContain('No audit entries');
  });
  ...
  it('AT10-04: null title falls back to default "Audit Trail"', () => {
    const html = renderToString(
      React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { title: null }),
    );
    expect(html).toContain('Audit Trail');
  });
});
...
describe('AT12 · Never-throw', () => {
  it('AT12-01: never throws with null items inside the entries array', () => {
    ...
    expect(() =>
      renderToString(React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { entries: bad }))
    ).not.toThrow();
  });
  ...
  it('AT12-03: never throws when timestamp is non-finite or a string', () => {
    ...
    expect(() =>
      renderToString(React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { entries }))
    ).not.toThrow();
  });
  ...
  it('AT12-04: never throws for edge combinations of null/undefined/empty props', () => {
    ...
    for (const props of cases) {
      expect(() =>
        renderToString(React.createElement<AuditTrailPanelProps>(AuditTrailPanel, props))
      ).not.toThrow();
    }
  });
  ...
  it('AT12-05: renders 100 entries with mixed valid and null items without throwing', () => {
    ...
    const html = renderToString(React.createElement<AuditTrailPanelProps>(AuditTrailPanel, { entries }));
