...
// ── Architecture guard for Phase X.4.5 (Reasoning Confidence Scoring) ────────
// Per this milestone's rules: deterministic confidence evaluation only — zero new conflict
// resolution, zero conflict-result modification, zero citation/explanation/answer generation,
// zero repository/KnowledgePlatform/provider/LLM/PromptBuilder/MCP/Tool Calling/Multi-Agent
// access. No src/knowledge/, src/ai/, src/mcp/, or src/conversation/ import. Checks run against
...
const NEW_FILES = [
  'src/reasoning/domain/confidenceEvaluationTypes.ts',
  'src/reasoning/application/confidenceEvaluationStage.ts',
]

describe('Architecture guard — Phase X.4.5 dependency direction', () => {
  ...
  it('confidenceEvaluationStage.ts imports only answerComposer.ts and reasoning\'s own domain types — never a repository, platform, orchestrator, engine, ruleEngine.ts, or conflictResolutionStage.ts directly', () => {
    const content = readCode('src/reasoning/application/confidenceEvaluationStage.ts')
  ...
  it('performs zero conflict resolution, citation/explanation/answer generation, or rule/threshold execution', () => {
    for (const file of NEW_FILES) {
      const content = readCode(file).toLowerCase()
      for (const forbidden of [
        'resolveconflicts(', 'resolvepair', 'groupbyconflictdimension', 'ismorerestrictive',
        'composedecision', 'formatcitations', 'citationformatter',
        'buildexplanation', 'evaluaterule(', 'evaluatethreshold(', 'detectexceptions',
        'searchknowledge', 'resolveknowledge(', 'detectintent',
      ]) {
  ...
  it('never mutates or reassigns fields on the input ConflictResolutionResult/RuleEvaluationResult (no assignment into their properties)', () => {
    const content = readCode('src/reasoning/application/confidenceEvaluationStage.ts')
    expect(content).not.toMatch(/conflictResolution\.\w+\s*=/)
    expect(content).not.toMatch(/ruleEvaluation\.\w+\s*=/)
  })

  it('evaluateConfidence deep-freezes its result', () => {
    const content = readCode('src/reasoning/application/confidenceEvaluationStage.ts')
    expect(content).toMatch(/Object\.freeze/)
    expect(content).toMatch(/deepFreeze/)
  })

  it('confidenceEvaluationTypes.ts reuses ConfidenceComponents rather than duplicating the scoring-result shape', () => {
    const content = readCode('src/reasoning/domain/confidenceEvaluationTypes.ts')
    expect(content).toMatch(/from ['"]\.\/reasoningTypes\.ts['"]/)
    expect(content).not.toMatch(/interface\s+ConfidenceComponents\b/)
    expect(content).not.toMatch(/interface\s+ConfidenceDeduction\b/)
  })

  it('does not export any private helper name from an earlier milestone (toAppliedDocument/roleByItemId construction is local to this file, not re-exported)', () => {
    const content = readCode('src/reasoning/application/confidenceEvaluationStage.ts')
    expect(content).not.toMatch(/export\s+function\s+toAppliedDocument/)
    expect(content).not.toMatch(/export\s+function\s+computeRoleByItemId/)
  })
})

describe('Architecture guard — frozen modules untouched (all of X.3.1 through X.4.4, and everything before)', () => {
