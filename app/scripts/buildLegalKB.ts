...
export function extractMetadata(file: DocFile, rawText: string): DocumentMetadata {
  ...
  // Body: strip h1 title and metadata fields; preserve h2+ for section-based chunking
  const body = rawText
    .replace(/^#(?!#).+$/gm, '')                     // remove h1 only — h2+ kept for chunkText splitAtSections
...
export function generateKBSource(entries: LegalEntry[]): string {
  // ponytail: keep JSON double-quote syntax — TypeScript accepts it and JSON.stringify
  // handles all escaping (apostrophes, backslashes, etc.) correctly.
